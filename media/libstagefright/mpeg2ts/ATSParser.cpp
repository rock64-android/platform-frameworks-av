...
struct ATSParser::Program : public RefBase {
    ...
    bool parsePID(
            ...
            ABitReader *br,uint32_t seekFlag,status_t *err, SyncEvent *event);
       ...
       if(mStreamType == 0x83 && player_type !=3){
        ...
        if (PES_packet_length != 0) {
           ...
           if(br->numBitsLeft() < dataLength * 8)
           {
            ...
            onPayloadData(
                    PTS_DTS_flags, PTS, DTS, br->data(), br->numBitsLeft()/8, event);
...
//            LOGE("PTS = %lld",PTS);
    ...
    while ((accessUnit = mQueue->dequeueAccessUnit()) != NULL) {
        ...
        if ((event != NULL) && !found && mQueue->getFormat() != NULL) {
            int32_t sync = 0;
            if (accessUnit->meta_data()->findInt32(kKeyIsSyncFrame, &sync) && sync) {
                int64_t timeUs;
                if (accessUnit->meta_data()->findInt64(kKeyTime, &timeUs)) {
                    found = true;
...
status_t ATSParser::feedTSPacket(const void *data, size_t size,
        SyncEvent *event) {
    return feedTSPacket(data, size, 0,event);
}
...
void ATSParser::createLiveProgramID(unsigned AudioPID,unsigned AudioType,unsigned VideoPID,unsigned VideoType)
{
    ...
    mPrograms.push(new Program(this,1,programMapPID,mLastRecoveredPTS));
...
status_t ATSParser::parsePID(
    ...
    for (size_t i = 0; i < mPrograms.size(); ++i) {
        ...
        if (mPrograms.editItemAt(i)->parsePID(
                    PID,  continuity_counter, payload_unit_start_indicator, br,seekFlag,&err, event)) {
            if (err != OK) {
                return err;
            }
