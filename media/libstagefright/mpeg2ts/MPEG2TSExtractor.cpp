...
#define DVB_FECTH_PACKET_NUM   50
       ...
       if(ret > 0)
       {
           for(int i =0; i< DVB_FECTH_PACKET_NUM; i++){
               ...
               {
                   uint32_t seekflag = 0;
                   mParser->feedTSPacket(ts_packets+i*kTSPacketSize, kTSPacketSize,seekflag);
               }
           }
       }
       else if(ret < 0)
       ...
       case GPU_STRM:
           {
            if(n==-2111 )
            {
                ALOGV("anothor thread has already got the data init_flag %d %2x%2x%2x%2x%2x%2x%2x%2x",init_flag,packet[0],packet[1],packet[2],packet[3],packet[4],packet[5],packet[6],packet[7]);
                if(init_flag==1)
                    return -2111;
                else
                {
