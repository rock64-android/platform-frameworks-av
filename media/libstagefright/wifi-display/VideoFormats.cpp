...
const size_t VideoFormats::invalidIndex = 0xFFFFFFFF;
...
bool VideoFormats::PickBestFormat(
    ...
    if(bestIndex != invalidIndex && (sinkSupported.isResolutionEnabled((ResolutionType)bestType, bestIndex)))
        first = false;
    else {
        for (size_t i = 0; i < kNumResolutionTypes; ++i) {
            for (size_t j = 0; j < 32; ++j) {
