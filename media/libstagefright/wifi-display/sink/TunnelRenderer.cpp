...
void TunnelRenderer::StreamSource::doSomeWork() {
    ...
    while (!mIndicesAvailable.empty()) {
		...
		do
		{
	        ...
	        CHECK_EQ((srcBuffer->size() % 188), 0u);
	        memcpy((void *)((int64_t)mem->pointer() + buffer_num), srcBuffer->data(), srcBuffer->size());
...
void TunnelRenderer::queueBuffer(const sp<ABuffer> &buffer) {
	...
	if(1)
	{
        ...
        ALOGD("update new start system time Us,%lld:%lld:%lld:%lld",(long long)curtime[0],(long long)curtime[1], (long long)tunnelrender_audio_start_time_us,(long long)tunnelrender_start_time_us);
							tunnelrender_start_time_us = curtime[0] - (curtime[1] - tunnelrender_audio_start_time_us);
					}	
